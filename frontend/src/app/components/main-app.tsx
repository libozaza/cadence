...
export function MainApp() {
  ...
  async function load(isRefresh = false) {
    ...
    try {
      ...
      if (history.predictions.length > 0) {
        setRiskScore(
          history.predictions[history.predictions.length - 1].risk_score,
        );
  ...
  const deviationScore =
    riskScore !== null ? Math.round(riskScore * 100) : null;

  const holdTimeData = toChartData(features, "hold_time_mean", 50, 150, "hold");
  const flightTimeData = toChartData(
    features,
    "flight_time_mean",
    100,
    400,
  ...
  const latencyTimeData = toChartData(
    features,
    "latency_time_mean",
    150,
    500,
        ...
        <div className="bg-card border-b border-border px-6 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            ...
            <span className="text-[15px] text-foreground">
              Keystrokes today: {rawKeystrokesToday.toLocaleString()}
            </span>
          </div>
        </div>
            ...
            <div className="space-y-6">
                  ...
                  {loading ? (
                    ...
                    <>
                      ...
                      <p className="text-sm text-muted-foreground mb-3">
                        Motor pattern deviation
                      </p>
                      <p className="text-base text-foreground text-center max-w-xs">
                        {scoreDescription}
                      </p>
                    </>
                  )}
                ...
                <div className="bg-card border border-border rounded-xl p-8 flex flex-col justify-center">
                  ...
                  <div className="space-y-4">
                    {featureMetrics.map((metric) => (
                      ...
                      >
                        <span className="text-[15px] text-foreground flex-shrink-0 w-44">
                          {metric.label}
                        </span>
              ...
              {features.length === 0 ? (
                <div className="bg-card border border-border rounded-xl p-12 text-center text-muted-foreground text-lg">
                  {loading
                    ? "Loading data…"
                    : "No daily data yet — keep typing and check back tomorrow after the nightly aggregation runs."}
                </div>
              ) : (
                <div className="grid grid-cols-3 gap-4">
                  ...
                  <div className="bg-card border border-border rounded-xl p-6 flex flex-col">
                    <h3 className="text-base mb-3" style={{ fontWeight: 600 }}>
                      Latency time (inter-key interval)
                    </h3>
                    <div className="flex items-center gap-4 mb-4">
                      <div className="flex items-center gap-2">
                        <div className="w-6 h-0.5 bg-accent rounded" />
          ...
          {activeView === "care" && (
            <div className="max-w-4xl">
              ...
              <p className="text-muted-foreground text-xl mb-10">
                Share your Cadence report with a movement disorder specialist.
              </p>
              <div className="space-y-5 mb-10">
                ...
                <div className="flex gap-4">
                  {["5 miles", "10 miles", "25 miles"].map((radius, idx) => (
                    <button
                      key={radius}
                      className={`px-8 py-4 rounded-full text-lg transition-colors ${idx === 1 ? "bg-accent text-accent-foreground" : "bg-muted/30 text-muted-foreground hover:bg-muted"}`}
                    >
                      {radius}
                    </button>
                  ))}
              ...
              <p className="text-muted-foreground text-lg text-center">
                Results shown are for informational purposes. Cadence does not
              ...
              <div className="space-y-10">
                  ...
                  <div className="bg-card border border-border rounded-xl p-7 flex items-center justify-between gap-6">
                    <div className="flex-1">
                      <p className="text-lg mb-2">
                        Background keystroke monitoring
                      </p>
                      <p className="text-muted-foreground text-[15px]">
                        Cadence monitors your typing quietly in the background
                      </p>
                    </div>
                ...
                <div className="pb-10 border-b border-border">
                  ...
                  <div className="space-y-5">
                    <div className="bg-card border border-border rounded-xl p-7 flex items-center justify-between gap-6">
                      <p className="text-lg">
                        Notify me if patterns are elevated
                      </p>
                      <button className="w-16 h-9 bg-accent rounded-full relative flex-shrink-0">
                        <div className="absolute right-1 top-1 w-7 h-7 bg-white rounded-full" />
                      </button>
                    ...
                    <div className="bg-card border border-border rounded-xl p-7">
                      ...
                      <input
                        ...
                        max="80"
                        defaultValue="50"
                        className="w-full mb-5 h-2"
                      />
                      <p className="text-[15px] text-muted-foreground mb-5 leading-relaxed">
                        You'll be notified if your motor pattern score stays
                        elevated for 3 or more consecutive weeks.
                      </p>
                      <p className="text-[15px] text-foreground leading-relaxed">
                        If your score stays above 50% for 3 weeks in a row,
                        Cadence will suggest speaking with a neurologist.
                      </p>
                    </div>
                  </div>
                </div>
                    ...
                    <div>
                      <button
                        onClick={async () => {
                          if (
                            window.confirm(
                              "This will permanently delete all your keystroke data and scores. This cannot be undone.",
                            )
                          ) {
                      ...
                      <p className="text-[15px] text-muted-foreground mt-3 text-center leading-relaxed">
                        This will permanently delete all keystroke data and
                        scores stored on your device. This cannot be undone.
                      </p>
                    </div>
                  </div>
                </div>
